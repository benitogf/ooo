...
func createUser(t *testing.T, server *ooo.Server, id int, name string) {
	user := TestUser{ID: id, Name: name}
	data, err := json.Marshal(user)
	require.NoError(t, err)

	server.Storage.Set(key.Build("users/*"), data)
}

func createPost(t *testing.T, server *ooo.Server, id int, title string) {
	post := TestPost{ID: id, Title: title}
	data, err := json.Marshal(post)
	require.NoError(t, err)
	server.Storage.Set(key.Build("posts/*"), data)
}

func createComment(t *testing.T, server *ooo.Server, id int, text string) {
	comment := TestComment{ID: id, Text: text}
	data, err := json.Marshal(comment)
	require.NoError(t, err)
	server.Storage.Set(key.Build("comments/*"), data)
}

func createLike(t *testing.T, server *ooo.Server, id int, userID int) {
	like := TestLike{ID: id, UserID: userID}
	data, err := json.Marshal(like)
	require.NoError(t, err)
	server.Storage.Set(key.Build("likes/*"), data)
}
...
func TestSubscribeMultiple2_BasicFunctionality(t *testing.T) {
	...
	go func() {
		client.SubscribeMultipleList2(
			ctx,
			client.Path{Protocol: "ws", Host: server.Address, Path: "users/*", Silence: true},
			client.Path{Protocol: "ws", Host: server.Address, Path: "posts/*", Silence: true},
...
func TestSubscribeMultiple3_BasicFunctionality(t *testing.T) {
	...
	go client.SubscribeMultipleList3(
		...
		client.Path{Protocol: "ws", Host: server.Address, Path: "users/*", Silence: true},
		client.Path{Protocol: "ws", Host: server.Address, Path: "posts/*", Silence: true},
		client.Path{Protocol: "ws", Host: server.Address, Path: "comments/*", Silence: true},
...
func TestSubscribeMultiple4_BasicFunctionality(t *testing.T) {
	...
	go client.SubscribeMultipleList4(
		...
		client.Path{Protocol: "ws", Host: server.Address, Path: "posts/*", Silence: true},
		client.Path{Protocol: "ws", Host: server.Address, Path: "comments/*", Silence: true},
		client.Path{Protocol: "ws", Host: server.Address, Path: "likes/*", Silence: true},
